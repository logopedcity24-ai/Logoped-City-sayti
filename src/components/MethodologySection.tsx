      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
          {METHODOLOGY_STEPS.map((step, idx) => {
            const title = language === 'uz' ? step.title : (step.titleRu || step.title);
            const description = language === 'uz' ? step.description : (step.descriptionRu || step.description);
            const duration = language === 'uz' ? step.duration : (step.durationRu || step.duration);
            const details = language === 'uz' ? step.details : (step.detailsRu || step.details);
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 pt-3 text-[11px] font-medium text-emerald-700 flex items-center">
                  <span>{language === 'uz' ? `Bosqich ${idx + 1} / 4` : `Этап ${idx + 1} / 4`}</span>
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-12 bg-gradient-to-r from-emerald-50 via-teal-50 to-slate-50 border border-emerald-200 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-start space-x-4">
            ...
            <div className="space-y-1">
              <h4 className="font-heading font-bold text-slate-900 text-base sm:text-lg">
                {t.methodology.noticeTitle}
              </h4>
              <p className="text-xs sm:text-sm text-slate-600 max-w-2xl leading-relaxed">
                {t.methodology.noticeText}
              </p>
            </div>
          </div>
          ...
          >
            {language === 'uz' ? 'Baholashga yozilish' : 'Записаться на оценку'}
          </button>
        </div>
