...
export const FaqSection: React.FC = () => {
  ...
  const filteredFaqs = FAQ_ITEMS.filter(item => {
    const q = (language === 'uz' ? item.question : (item.questionRu || item.question)).toLowerCase();
    const a = (language === 'uz' ? item.answer : (item.answerRu || item.answer)).toLowerCase();
    const c = (language === 'uz' ? item.category : (item.categoryRu || item.category)).toLowerCase();
    ...
    <section id="faq" className="py-16 sm:py-24 bg-slate-50 border-b border-slate-200/60">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 mb-8 max-w-xl mx-auto relative">
          ...
          <input
            type="text"
            placeholder={language === 'uz' ? 'Savolingiz bo‘yicha qidiring (masalan: baholash, CBO)...' : 'Поиск по вопросам (например: оценка, СБО)...'}
            value={searchQuery}
        ...
        <div className="space-y-3">
          {filteredFaqs.map((faq) => {
            ...
            const question = language === 'uz' ? faq.question : (faq.questionRu || faq.question);
            const answer = language === 'uz' ? faq.answer : (faq.answerRu || faq.answer);
            const category = language === 'uz' ? faq.category : (faq.categoryRu || faq.category);
          ...
          {filteredFaqs.length === 0 && (
            <div className="text-center py-8 text-slate-500 text-sm">
              {language === 'uz' 
                ...
                : 'По вашему запросу ничего не найдено. Вы можете позвонить нам напрямую для консультации.'}
            </div>
          )}
        </div>
